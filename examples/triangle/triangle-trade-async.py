...
async def find_trade_chance(exchange,base='EOS',quote='BTC',mid='USDT'):
    ...
    try:
        book_p1 = await exchange.fetch_order_book(p1_trade_pair)
        # time.sleep(delay)
        await asyncio.sleep(delay)
        book_p2 = await exchange.fetch_order_book(p2_trade_pair)
        # time.sleep(delay)
        book_p3 = await exchange.fetch_order_book(p3_trade_pair)
        await asyncio.sleep(delay)
    except Exception as e:
